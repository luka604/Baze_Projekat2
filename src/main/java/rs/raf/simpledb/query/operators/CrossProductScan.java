...
/**
 * The scan class corresponding to the <i>Cross-Product</i> relational
 * algebra operator.
 * @author Edward Sciore
 */
public class CrossProductScan implements Scan {
   private Scan s1, s2;
   
   /**
    * Creates a Cross-product scan having the two underlying scans.
    * @param s1 the LHS scan
    * @param s2 the RHS scan
    */
   public CrossProductScan(Scan s1, Scan s2) {
      this.s1 = s1;
      this.s2 = s2;
      s1.next();
   }
   ...
   public Constant getVal(String fldname) {
       //System.out.println("CrossProduct GetVal: field="+fldname);
