...
import rs.raf.simpledb.query.operators.SelectionScan;
import rs.raf.simpledb.record.Schema;

/** The Plan class corresponding to the <i>Selection</i>
  * relational algebra operator.
  * @author Edward Sciore
  */
public class SelectionPlan implements Plan {
   private Plan p;
   private Predicate pred;
   
   /**
    * Creates a new Selection node in the query tree,
    * having the specified subquery and predicate.
    * @param p the subquery
    * @param pred the predicate
    */
   public SelectionPlan(Plan p, Predicate pred) {
      this.p = p;
      this.pred = pred;
   }
   
   /**
    * Creates a Selection scan for this query.
    * @see simpledb.query.Plan#open()
    */
   public Scan open() {
      Scan s = p.open();
      return new SelectionScan(s, pred);
   }
   ...
   @Override
	public void printPlan(int indentLevel) {
		...
		System.out.println("-".repeat(indentLevel)+"-> SELECTION ("+ pred.toString()+") OF ->");
